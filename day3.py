...
for dis in firstwire:
    ad(dis[0],int(dis[1:]),first)

current=(0,0)  
for dis in secondwire:
    ad(dis[0],int(dis[1:]),second)

#part_1
ans=[] 
for _ in first.intersection(second) - {(0,0)}:
    a,b=_
...
path1={}
path2={}
inter=first.intersection(second) - {(0,0)}
def ad2(k,n,t,cho,path):
    global current
    global lul
    x,y=current
    if k=='R':
...
lul=0
for dis in firstwire:
    ad2(dis[0],int(dis[1:]),first,lul,path1)
    
current =(0,0)
lul=0
for dis in secondwire:
    ad2(dis[0],int(dis[1:]),second,lul,path2)

ans2=[]    
for inte in path1:
    ans2.append(path1[inte]+path2[inte])
print(min(ans2),"part2")